...
BASE_CFLAGS += `pkg-config --libs-only-L --libs --cflags vorbisfile` `sdl-config --cflags`
...
ifeq ($(TYPE),release)
LDFLAGS = -lm 'sdl-config --libs` -lpthread -lrt
else
LDFLAGS = -ggdb `sdl-config --libs` -lm -lpthread -lrt
endif
